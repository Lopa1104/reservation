 ...
 */
class Reservation_Deposits_Orders
{
    ...
    function adjust_second_payment_status($order_id)
    {
        ...
        if ($is_order_editor) {
            ...
            if ($order->get_type() !== 'reservation_payment' && $order_has_deposit === 'yes') {
    ...
    {
        $order = wc_get_order($order_id);
        if ($order && $order->get_type() === 'reservation_payment' && $order->get_meta('_wc_deposits_payment_type', true) === 'deposit') {
    ...
    {
        $order = wc_get_order($order_id);
        if ($order && $order->get_type() === 'reservation_payment') {
            $parent = wc_get_order($order->get_parent_id());
            if ($parent) {
    ...
    {
        if ($order->get_type() === 'reservation_payment') return false;
    ...
    function prevent_user_trash_partial_payments($trash, $post)
    {
        ...
        if (is_object($post) && $post->post_type === 'reservation_payment') {
    ...
    function output_myaccount_partial_payments_summary($order)
    {
        ...
        if (is_account_page() && $order_has_deposit === 'yes' && apply_filters('`wc_deposits_myaccount_show_partial_payments_summary`', true, $order)) {
            ...
            wc_get_template(
                'order/reservation-deposits-partial-payments-summary.php', array(
    ...
    function output_parent_order_summary($partial_payment)
    {
        ...
        if ($partial_payment->get_type() === 'reservation_payment') {
            wc_get_template('order/reservation-deposits-order-summary.php', array('partial_payment' => $partial_payment, 'order_id' => $partial_payment->get_parent_id()), '', WC_DEPOSITS_TEMPLATE_PATH);
        }
    ...
    function checkout_payment_url($url, $order)
    {
        ...
        $order_has_deposit = $order->get_meta('_wc_deposits_order_has_deposit', true);
        if ($order_has_deposit === 'yes' && $order->get_type() !== 'reservation_payment') {
    ...
    function redirect_legacy_links()
    {
        ...
        if (!empty($wp->query_vars['order-pay'])) {
            ...
            if ($order && $order->needs_payment() && $order->get_type() !== 'reservation_payment' && $order_has_deposit === 'yes') {
    ...
    public function needs_payment($needs_payment, $order, $valid_statuses)
    {
        ...
        if (is_checkout_pay_page() && $order->get_type() === 'reservation_payment' && $order->get_meta('_wc_deposits_payment_type', true) !== 'deposit') {
    ...
    public function valid_order_statuses_for_payment_complete($statuses, $order)
    {
        ...
        if ($order->get_type() !== 'reservation_payment' && get_option('wc_deposits_remaining_payable', 'yes') === 'yes') {
    ...
    public function get_order_item_totals($total_rows, $order)
    {
        ...
        if ($order_has_deposit)  :
            ...
            if ($future_payment_amount_text === false) {
