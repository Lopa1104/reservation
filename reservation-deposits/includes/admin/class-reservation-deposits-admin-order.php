 ...
 *
 */
class Reservation_Deposits_Admin_Order
{
    /**
    ...
    public function __construct()
    {
        ...
        // Hook the order admin pasaved_order_itemsge
    ...
    function remove_statuses_for_partial_payment()
    {
        ...
        if (function_exists('get_current_screen')) {
            ...
            if ($screen)
                $partial_payment_editor = $screen->id === 'reservation_payment';
        }
    ...
    function recalculate_deposit_callback()
    {
                    ...
                    if (isset($form_data['wc_deposits_deposit_enabled_' . $order_item->get_id()]) && $form_data['wc_deposits_deposit_enabled_' . $order_item->get_id()] === 'yes') {
                        ...
                        if ($item_amount_type === 'payment_plan') {
                            ...
                            foreach ($plan_lines['payment-plan'] as $plan_id => $plan_detail) {
                                ...
                                $schedule[$payment_date]['amount'] = ($order_item->get_subtotal('edit') / 100 * $plan_detail['percentage']);
        ...
        if ($deposit_enabled) {
            ...
            $fees_handling = isset($form_data['wc_deposits_fees_handling']) ? $form_data['wc_deposits_fees_handling'] : get_option('wc_deposits_fees_handling');
            $taxes_handling = isset($form_data['wc_deposits_taxes_handling']) ? $form_data['wc_deposits_taxes_handling'] : get_option('wc_deposits_taxes_handling');
            $shipping_handling = isset($form_data['wc_deposits_shipping_handling']) ? $form_data['wc_deposits_shipping_handling'] : get_option('wc_deposits_shipping_handling');
            $shipping_taxes_handling = isset($form_data['wc_deposits_shipping_taxes_handling']) ? $form_data['wc_deposits_shipping_taxes_handling'] : get_option('wc_deposits_shipping_taxes_handling');
            ...
            $deposit_percentage = round($recalculated_deposit * 100 / floatval($order->get_subtotal() - $order->get_total_discount()),1);
            ...
            $discount_from_deposit = isset($form_data['wc_deposits_coupons_handling']) ? $form_data['wc_deposits_coupons_handling'] : get_option('wc_deposits_coupons_handling','second_payment');
            ...
            if ($discount_from_deposit === 'deposit') {
                ...
                $deposit_discount = $order->get_total_discount() / 100 * $deposit_percentage;
                $recalculated_deposit -= $deposit_discount;

            }
        }
        $remaining_discount = $discount_total - $deposit_discount;
        ...
        if ($deposit_enabled && is_numeric($recalculated_deposit) && $recalculated_deposit < floatval($order->get_total())) {
            ...
            $remaining_amounts['discounts'] = $remaining_discount;


            $partial_payments_schedule = $this->build_payment_schedule($remaining_amounts, $order, $recalculated_deposit, $cart_items_deposit);
    ...
    private function build_payment_schedule($remaining_amounts, $order, $deposit, $cart_items_deposit)
    {
        ...
        $unlimited = array(
            'id' => '',
            'title' => esc_html__('Future Payments', 'woocommerce-deposits'),
            'type' => 'second_payment',
            'total' => 0.0,
        );
        $payment_date = current_time('timestamp');
        ...
        if (wcdp_checkout_mode()) {
            ...
            if ($amount_type === 'payment_plan') {
                ...
                if (!empty($second_payment_due_after) && is_numeric($second_payment_due_after)) {
                    ...
                    $schedule[$timestamp]['total'] = floatval($order->get_subtotal('edit') - $cart_items_deposit);
                    if (!isset($schedule[$timestamp]['type'])) $schedule[$timestamp]['type'] = 'second_payment';

                } else {

                    $unlimited['total'] = floatval($order->get_subtotal('edit') - $cart_items_deposit);
                ...
                if (!empty($deposit_meta) && $deposit_meta['enable'] === 'yes' && isset($deposit_meta['deposit'])) {
                    ...
                    if (isset($deposit_meta['payment_schedule'])) {
                        ...
                        foreach ($deposit_meta['payment_schedule'] as $timestamp => $payment) {
                            ...
                            $schedule[$timestamp]['total'] += round($payment['amount'], wc_get_price_decimals());
                        }
        ...
        foreach ($schedule as $payment_key => $payment) {
            ...
            foreach ($remaining_amounts as $amount_key => $remaining_amount) {
                ...
                if ($amount_key !== 'fees' && $remaining_amount <= 0) continue; //workaround negative fees
    ...
    function recalculate_deposit_button($order)
    {
        ...
        <button type="button" data-msg="<?php echo $recalculate_deposit_msg; ?>"
                ...
                class="button button-primary"><?php esc_html_e('Recalculate Deposit', 'woocommerce-deposits'); ?></button>
     ...
     * @return array
     */
    public function resend_order_emails_available($emails_available)
    {
     ...
     * @return mixed
     */
    public function order_actions($emails_available)
    {
        ...
        if ($order->get_type() === 'reservation_payment') return $emails_available;
    ...
    public function admin_order_item_headers()
    {
        ...
        if (!$order || $order->get_type() === 'reservation_payment') return;


        ?>
        <th class="deposit-paid"><?php esc_html_e('Deposit', 'woocommerce-deposits'); ?></th>
        <th class="deposit-remaining"><?php esc_html_e('Future Payments', 'woocommerce-deposits'); ?></th>
        <?php
    }
    ...
    { 
        $order = wc_get_order($order_id);
        if ($order->get_type() === 'reservation_payment') return;
        $order_has_deposit = $order->get_meta('_wc_deposits_order_has_deposit', true);
        ...
        ?>
        <tr>
            <td class="label"><?php echo wc_help_tip(esc_html__('Note: Deposit amount is affected by settings for fees, taxes & shipping handling', 'woocommerce-deposits')); ?><?php esc_html_e('Deposit', 'woocommerce-deposits'); ?>
                :
            </td>
            <td width="1%"></td>
        ...
        <tr class="wcdp-remaining">
            <td class="label"><?php esc_html_e('Future Payments', 'woocommerce-deposits'); ?>:</td>
            <td width="1%"></td>
    ...
    function original_order_metabox()
    {
        ...
        <a class="button btn" href="
                  <?php echo esc_url($parent->get_edit_order_url()); ?> "> <?php esc_html_e('View', 'woocommerce-deposits'); ?> </a>
        <?php
        ...
        <label for="partial_payment_datepicker">
            ...
            <?php esc_html_e('Due date:', 'woocommerce-deposits') ?>
        </label>
    ...
    function request_query($query_vars)
    {
        ...
        if (isset($query_vars['post_type']) && $query_vars['post_type'] === 'reservation_payment') {
            // Status.
    ...
    function process_payment_date_datepicker_values($post_id)
    {
        ...
        if (!$partial_payment || $partial_payment->get_type() !== 'reservation_payment') return;
        ...
        if (!empty($partial_payment_date)) {
            ...
            if (isset($payment_schedule[$new_timestamp])) {
                ...
                if ($payment_schedule[$new_timestamp]['id'] !== $partial_payment->get_id()) {
                    wp_die(sprintf(wp_kses(__('Error saving. A payment with this date already exist. <a href="%s"> Return </a>', 'woocommerce-deposits'),array('a' =>array('href'))), $partial_payment->get_edit_order_url()));
                }
